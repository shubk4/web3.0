...
// You can edit this contract as much as you want. A template is provided here and you can change the function names and implement anything else you want, but the basic tasks mentioned here should be accomplished.
    ...
    // They take in the address of a creditor as arguments. viewDues returns a uint256 corresponding to the due amount, 
    // and does not modify any state variables. settleDues returns a bool, true if the dues were settled and false otherwise.
    ...
    function viewDues(address creditor) public view isOwner returns(uint256){
        uint256 ret = loans[creditor];
        return ret;
    }
    
    function settleDues(address creditor) public isOwner returns(bool){
        uint256 loanAmount = viewDues(creditor);
        if(getOwnerBalance()>loanAmount){
            sendCoin(creditor, loanAmount , Owner);
            loans[creditor]=0;
            return true;
        }
        else{
